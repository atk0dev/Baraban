...
namespace StoreWebApp
{
    public static class WebApiConfig
    {
        public static void Register(HttpConfiguration config)
        {
            ...
            config.Routes.MapHttpRoute(
              name: "DetailsApi",
              routeTemplate: "api/v1/details/{id}",
              defaults: new { controller = "details", id = RouteParameter.Optional }
          );

            config.Routes.MapHttpRoute(
                name: "GroupApi",
                routeTemplate: "api/v1/groups/{id}",
                defaults: new { controller = "groups", id = RouteParameter.Optional }
            );

            config.Routes.MapHttpRoute(
              name: "CatalogDataApi",
              routeTemplate: "api/v1/catalogdata/{id}",
              defaults: new { controller = "catalogdata", id = RouteParameter.Optional }
          );
